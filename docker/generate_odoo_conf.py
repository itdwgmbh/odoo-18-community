...
def get_env_or_file(var_name, default=""):
    ...
    if file_path:
        try:
            ...
            print(
                f"Error: Cannot read secret file '{file_path}' specified by {file_var}: {e}",
                file=sys.stderr,
            )
            sys.exit(1)
...
ODOO_CONFIG = {
    "options": {
        ...
        "db_host": os.environ.get(
            "DB_HOST", os.environ.get("DB_PORT_5432_TCP_ADDR", "db")
        ),
        ...
        "db_user": os.environ.get(
            "DB_USER",
            os.environ.get(
                "DB_ENV_POSTGRES_USER", os.environ.get("POSTGRES_USER", "odoo")
            ),
        ),
        "db_password": get_env_or_file(
            "DB_PASSWORD",
            os.environ.get(
                "DB_ENV_POSTGRES_PASSWORD", os.environ.get("POSTGRES_PASSWORD", "odoo")
            ),
        ),
        ...
        # Master password
        "admin_passwd": get_env_or_file(
            "ODOO_MASTER_PASSWORD", os.environ.get("ODOO_ADMIN_PASSWD", "")
        ),
        ...
        # Memory limits
        "limit_memory_hard": os.environ.get(
            "ODOO_LIMIT_MEMORY_HARD", "4294967296"
        ),  # 4GB
        "limit_memory_soft": os.environ.get(
            "ODOO_LIMIT_MEMORY_SOFT", "3221225472"
        ),  # 3GB
        ...
        # Logging
        "log_handler": os.environ.get(
            "ODOO_LOG_HANDLER", "['werkzeug:CRITICAL','odoo:WARNING']"
        ),
        ...
        if os.environ.get("ODOO_CONFIG_DEBUG", "False").lower() in ("true", "1", "yes"):
            ...
            with open(output_path, "r") as f:
                for line in f:
                    ...
                    if (
                        any(key in line.lower() for key in sensitive_keys)
                        and "=" in line
                    ):
